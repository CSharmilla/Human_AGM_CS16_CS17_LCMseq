...
for (nm in names(results_wald)) {
  write.xlsx(
    results_wald[[nm]],
    file = here("output", "excel_sheets",paste0(nm, "_DEA.xlsx")),
    overwrite = TRUE
  )
}
...
for (nm in names(dds)) {
  write.xlsx(
    counts(dds[[nm]], normalized = TRUE),
    file = here("output", "excel_sheets",paste0(nm, "_normalised_counts.xlsx")),
    overwrite = TRUE
  )
}
...
# saving count matrices for liana python script
ifelse(!dir.exists(here("python_input")),
       dir.create(here("python_input")),
       "Directory Exists")
# save csvs
for (name in names(counts)) {
  ...
  # clean
  mat <- mat[rownames(mat) != "" & !is.na(rownames(mat)), ]
  mat <- mat[!duplicated(rownames(mat)), ]
  
  write.csv(mat,
            file = file.path(here("python_input"), paste0(name, ".csv")),
            quote = FALSE)
}
